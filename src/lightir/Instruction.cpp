...
CallInst::CallInst(Function *func, std::vector<Value *> args, BasicBlock *bb)
    ...
    for (unsigned i = 0; i < args.size(); i++) {
        ...
        std::string functioninfo = func->print();
        std::cout << functioninfo << std::endl;//func_type->get_param_type(i), args[i]->get_type();
        std::string typeinfo = func_type->get_param_type(i)->print();
...
PhiInst::PhiInst(Type *ty, std::vector<Value *> vals,
    ...
    : BaseInst<PhiInst>(ty, phi) {  //这里传进来的参数没有bb，也就保证了不会执行在块尾插入的指令
