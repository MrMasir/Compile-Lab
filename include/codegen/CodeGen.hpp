...
/*这里的`= ASMInstruction::Instruction`表示如果调用`append_inst`函数时没有
提供第三个参数`ty`的值，那么它将默认为`ASMInstruction::Instruction`。*/
        auto content = std::string(inst) + " ";
        for (const auto &arg : args) {
            content += arg + ", ";
        }
        content.pop_back(); //.pop_back():用于删去容器中的最后一个元素
        content.pop_back();
        output.emplace_back(content, ty);//.emplace_back()用于向容器末尾添加元素
    }
